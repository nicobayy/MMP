...
def analyze_token(pair: dict, cfg: dict) -> tuple[float, list[str], dict]:
    ...
    if h24 < t["min_price_change_h24_pct"]:
        score -= 30
        notes.append(f"dump h24 {h24:.0f}%")
    elif h24 > t["max_price_change_h24_pct"]:
        score -= 30
        notes.append(f"parabolic h24 +{h24:.0f}% (telat)")
    elif 0 < h24 < 200:
        score += 10
        notes.append(f"momentum h24 sehat +{h24:.0f}%")
    if h1 < t["min_price_change_h1_pct"]:
        score -= 15
        notes.append(f"h1 lemah {h1:.0f}%")
    if vol_h1 >= t["min_volume_h1_usd"]:
        score += 10
        notes.append(f"vol h1 ${vol_h1:,.0f} aktif")
    else:
        score -= 10
        notes.append(f"vol h1 kecil ${vol_h1:,.0f}")
    ...
    if buys + sells > 0:
        ...
        if br >= 0.55:
            score += 10
            notes.append(f"buy pressure {br:.0%}")
        elif br < 0.45:
            score -= 10
            notes.append(f"sell pressure {br:.0%}")
